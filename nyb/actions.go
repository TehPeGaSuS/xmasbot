...
func (bot *Settings) addTriggers() {
	...
	irc.AddTrigger(kitty.Trigger{
		...
		Action: func(b *kitty.Bot, m *kitty.Message) {
			b.Reply(m, "Source code: https://github.com/TehPeGaSuS/xmasbot")
		},
	})
	...
	irc.AddTrigger(kitty.Trigger{
		Condition: func(b *kitty.Bot, m *kitty.Message) bool {
			return m.Command == "PRIVMSG" &&
				...
				normalize(m.Content) == bot.Prefix+"xmas"
		},
		Action: func(b *kitty.Bot, m *kitty.Message) {
	...
	irc.AddTrigger(kitty.Trigger{
		...
		Action: func(b *kitty.Bot, m *kitty.Message) {
			...
			if now().UTC().Add(dur).After(bot.target) {
				b.Reply(m, fmt.Sprintf("No more next, Christmas is here AoE", bot.target.Year()))
				return
			}
	...
	irc.AddTrigger(kitty.Trigger{
		...
		Action: func(b *kitty.Bot, m *kitty.Message) {
			...
			b.Reply(m, fmt.Sprintf("%d timezone%s remaining. %.2f%% are in the Christmas day", bot.remaining, plural, pct))
		},
	})
	...
	irc.AddTrigger(kitty.Trigger{
		Condition: func(b *kitty.Bot, m *kitty.Message) bool {
			return m.Command == "PRIVMSG" &&
				strings.HasPrefix(normalize(m.Content), bot.Prefix+"xmas ")
		},
		Action: func(b *kitty.Bot, m *kitty.Message) {
